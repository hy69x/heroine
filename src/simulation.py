...
def generate_input_file(
    ...
    lines = [
        "# LAMMPS Input file generated by HEROINE (CHONS Forcefield)",
        "",
        "units real",
        "atom_style full",
        "boundary p p p",
    ...
    try:
        ...
        if log_file.exists():
            with open(log_file, "r") as f:
                # Read last 20 lines for errors
                lines = f.readlines()[-20:]
                for line in lines:
                    if "ERROR" in line:
                        print(f"\n[!] LAMMPS ERROR DETECTED: {line.strip()}")
                        sys.exit(1)
                        
        if process.returncode != 0:
            print(f"\n[!] LAMMPS failed with exit code {process.returncode}")
            sys.exit(1)
